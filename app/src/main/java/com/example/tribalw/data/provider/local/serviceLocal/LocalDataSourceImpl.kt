package com.example.tribalw.data.provider.local.serviceLocal
...
class LocalDataSourceImpl @Inject constructor(private val localDao: LocalServiceInterfaceNote_Dao) : LocalDataSourceInterface{

    override suspend fun saveNoteRoom(noteEntity: NoteEntity) {
        localDao.saveNoteRoomDao(noteEntity = noteEntity)
    }

    override suspend fun getAllNoteRoom(): Resource<List<NoteEntity>> {
        return Resource.Success(localDao.getAllNoteRoomDao())
    }
