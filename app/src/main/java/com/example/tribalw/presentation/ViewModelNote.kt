package com.example.tribalw.presentation
...
@HiltViewModel
class ViewModelNote @Inject constructor(private val userCase: UserCaseNoteContract) : ViewModel() {

    //erick.espejel@tribalworldwide.gt
    //enviar correo del repo

    fun saveNotesEntityRoomVM(noteEntity: NoteEntity) {
        viewModelScope.launch {
            userCase.saveNoteRoomContract(noteEntity)
        }
    }

    fun getAllNotesRoomVM() = liveData(Dispatchers.IO) {
        emit(Resource.Loading())
        try {
            emit(userCase.getAllNoteRoomContract())
        } catch (e: Exception) {
